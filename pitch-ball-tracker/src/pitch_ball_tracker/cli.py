...
@click.option("--config", "-c", "config_path",
              default="configs/default.yaml", show_default=True,
              type=click.Path(exists=True),
              help="Path to YAML config file.")
# --- model overrides ---
@click.option("--half-precision/--no-half-precision", default=None,
              help="Enable FP16 half-precision inference.")
@click.option("--gpus", default=None,
              help="Comma-separated GPU IDs to use (e.g. '0' or '0,1').")
@click.option("--sam3-checkpoint", default=None, type=click.Path(),
              help="Path to SAM3 checkpoint (default: auto-download).")
# --- inference overrides ---
@click.option("--tiled/--no-tiled", default=None,
              help="Enable tiled inference for high-resolution frames.")
@click.option("--tile-size", default=None, type=int,
              help="Tile size in pixels when tiled inference is enabled.")
@click.option("--confidence", default=None, type=float,
              help="SAM3 detection confidence threshold.")
# --- tracking overrides ---
@click.option("--use-reid/--no-reid", default=None,
              help="Enable ReID-based re-identification.")
@click.option("--max-age", default=None, type=int,
              help="Max frames a track survives without a detection.")
def main(
    input_path: str,
    output_dir: str,
    config_path: str,
