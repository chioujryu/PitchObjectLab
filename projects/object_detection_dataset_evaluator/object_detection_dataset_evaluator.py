...
def apply_sahi_recheck(
    ...
    crop_size = int(
        recheck_cfg.get(
            "crop_size",
            max(int(sahi_cfg.get("slice_width", image.width)), int(sahi_cfg.get("slice_height", image.height))),
        )
    )
    second_conf = float(
    ...
    for prediction in predictions:
        ...
        fused_score = first_weight * float(prediction.get("score", 0.0)) + second_weight * float(
...
def run_inference(
    ...
    with ProcessPoolExecutor(max_workers=len(chunks)) as executor:
        ...
        for index, records in enumerate(chunks):
            ...
            futures.append(
                executor.submit(inference_worker, records, config, str(output_dir), device, worker_visual_ids)
            )
...
def run_evaluation(
    ...
    summary: dict[str, Any] = {
        **coco_summary,
        ...
        "avg_inference_seconds_per_image": float(np.mean([row["elapsed_seconds"] for row in stats_rows]))
        if stats_rows
        else 0.0,
    }
