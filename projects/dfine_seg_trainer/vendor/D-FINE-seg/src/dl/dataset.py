    ...
    def _init_augs(self, cfg) -> None:
        ...
        if self.mode == "train":
            augs = [
                ...
                A.MotionBlur(blur_limit=7, p=getattr(cfg.train.augs, "motion_blur", 0.0)),
                A.GaussNoise(p=cfg.train.augs.noise, std_range=(0.1, 0.2)),
                A.ISONoise(p=getattr(cfg.train.augs, "iso_noise", 0.0)),
                A.CLAHE(p=getattr(cfg.train.augs, "clahe", 0.0)),
                A.Sharpen(p=getattr(cfg.train.augs, "sharpen", 0.0)),
                A.ImageCompression(quality_range=(65, 100), p=getattr(cfg.train.augs, "compression", 0.0)),
                A.Downscale(scale_range=(0.5, 0.9), p=getattr(cfg.train.augs, "downscale", 0.0)),
                A.Affine(
                    rotate=[90, 90],
                ...
                A.HueSaturationValue(
                    hue_shift_limit=int(float(getattr(cfg.train.augs, "hsv_h", 0.0)) * 180),
                    sat_shift_limit=int(float(getattr(cfg.train.augs, "hsv_s", 0.0)) * 255),
                    val_shift_limit=int(float(getattr(cfg.train.augs, "hsv_v", 0.0)) * 255),
                    p=max(
                        float(getattr(cfg.train.augs, "hsv_h", 0.0)),
                        float(getattr(cfg.train.augs, "hsv_s", 0.0)),
                        float(getattr(cfg.train.augs, "hsv_v", 0.0)),
                    ),
                ),
                A.ChannelShuffle(p=getattr(cfg.train.augs, "bgr", 0.0)),
                A.GridDropout(ratio=0.5, p=getattr(cfg.train.augs, "grid_dropout", 0.0)),
                A.RandomShadow(p=getattr(cfg.train.augs, "random_shadow", 0.0)),
                A.RandomRain(p=getattr(cfg.train.augs, "random_weather", 0.0)),
                A.Rotate(
