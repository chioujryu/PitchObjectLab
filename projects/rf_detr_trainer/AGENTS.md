...
21. RF-DETR test and inference speedups must use real batched `model.predict([...])` paths, not only larger YAML values. Default to balanced batches (`test.batch_size`/`sahi.batch_size` 4, `inference.batch_size`/video detection batches 8), keep batch sizes configurable, and downshift on CUDA OOM before failing.
